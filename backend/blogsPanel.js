...
// ✅ Add these endpoints to your blogsPanel.js file

// Get all users endpoint (for UserManagement component)
...
// Validate JWT token endpoint (already in your code but ensure it's working)
app.get("/api/auth/validate-token", authenticateToken, async (req, res) => {
  try {
    // The authenticateToken middleware has already validated the token
    const user = await User.findById(req.user.id);
...
// ✅ Add a health check endpoint for ServerPing component
...
// ✅ Add catch-all 404 handler AFTER all your routes
app.all('*', (req, res) => {
